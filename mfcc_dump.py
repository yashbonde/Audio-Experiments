...
def paths_to_data(paths, word2id, unk = False):
	data_1 = np.zeros(shape = (len(paths), 98, 12))
	data_2 = np.zeros(shape = (len(paths), 98, 40))
	labels = []
	indexes = []
	for i in tqdm(range(len(paths))):
		f = paths[i]
		audio, fb = get_features(paths[i])
		if audio.shape != (98,12) or fb.shape != (98, 40):
			indexes.append(i)
		elif audio.shape == (98, 12) and fb.shape == (98, 40):
			data_1[i] = audio
			data_2[i] = fb
